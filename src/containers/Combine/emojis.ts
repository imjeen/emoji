import { EMOJI_API_PATH } from '@/constants';

type TypeEmojis = [number[], string, string][];

const emojiData: TypeEmojis = [
 ...
 * 获取组合数据
 *
 * @param {TypeEmojis[0]} emoji1
 * @param {TypeEmojis[0]} emoji2
 */
export function getCombineData(emoji1: TypeEmojis[0], emoji2: TypeEmojis[0]) {
  const u1 = emoji1[0].map(c => 'u' + c.toString(16)).join('-'),
    u2 = emoji2[0].map(c => 'u' + c.toString(16)).join('-');
  const pathname = `${emoji1[2]}/${u1}`;
